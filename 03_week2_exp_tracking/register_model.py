...
def train_and_log_model(data_path, params):
    ...
    with mlflow.start_run() as run:
        ...
        # Log the model artifact (important for registering)
...
def run_register_model(data_path: str, top_n: int):
    ...
    # Step 1: Retrieve top_n model runs from HPO experiment
    ...
    # Step 2: Re-train & log top models to "random-forest-best-models" experiment
    for run in hpo_runs:
        train_and_log_model(data_path=data_path, params=run.data.params)

    # Step 3: Retrieve the best run based on lowest test_rmse
    ...
    print(f"✅ Registered model from run_id: {run_id} with test RMSE: {best_run.data.metrics['test_rmse']}")
